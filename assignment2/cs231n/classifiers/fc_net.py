...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dx2 ,grads['W2'], grads['b2'] = affine_backward(dx1, self.cache['out'])
        _,grads['W1'], grads['b1'] = affine_relu_backward(dx2, self.cache['hidden'])
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
            fc, fc_cache[str(i+1)] = affine_forward(X, self.params['W'+str(i+1)], self.params['b'+str(i+1)])
            if self.use_batchnorm:
                bn, bn_cache[str(i+1)] = batchnorm_forward(fc, self.params['gamma'+str(i+1)], self.params['beta'+str(i+1)], self.bn_params[i])
                rl, rl_cache[str(i+1)] = relu_forward(bn)
            else:
                rl, rl_cache[str(i+1)] = relu_forward(fc)
                
            if self.use_dropout:
                rl, dr_cache[str(i+1)] = dropout_forward(rl, self.dropout_param)
        ...
        for i in range(self.num_layers-1, 0, -1):
            if self.use_dropout:
                dx_last = dropout_backward(dx_last, dr_cache[str(i)])
                
            drelu = relu_backward(dx_last, rl_cache[str(i)])
